...
pub const CURRENT_BLOCK_HEIGHT: u64 = 951200;
...
pub fn generate_address(prefix: &str) -> String {
    ...
    let target_len = 32usize;
    let prefix_len = prefix.len();
    if prefix_len >= target_len {
        return prefix.chars().take(target_len).collect();
    }

    let remaining = target_len - prefix_len;
...
pub fn extract_tx_version(raw_tx_hex: &str) -> Result<u32, String> {
    ...
    for i in 0..4 {
        let byte_str = &version_hex[i * 2..i * 2 + 2];
        bytes[i] = u8::from_str_radix(byte_str, 16).map_err(|_| "Hex decode error".to_string())?;
    }
